...
### 7.1 范围变更记录：Slot Review 提前进入 P0

**Slot Review 原列于本节，且在 §28 的路线图中属于 P2。现已提前进入 P0**，
规格见 FR-REVIEW-001..004。

变更依据是一次针对审核能力的实测（29 条盲测用例、三种提问架构），
它确认了两件事：其一，与产线同档的模型能可靠检出部分缺陷且**对真实产物零误判**，
因此该能力可以无人值守运行；其二，它对另一部分判据**完全检不出**，
因此它**不是质量保证**，FR-REVIEW-004 由此成为一条正确性要求。

提前的前提是「基础结构槽生产闭环稳定」这一条已经满足。

**术语澄清**：本节仍列为非目标的 `Finding`，指旧权威审核体系中那个
与 Repair Grant、Scope Expansion、Evidence 相互耦合的重量级实体
（另见 §28 P2 的同名提醒）。FR-REVIEW-002 中的 **Review Finding** 是一个
独立的轻量概念——一段引文加一句问题说明，必须通过确定性引文校验才被采纳，
不引入授权、评分、范围协商等任何机制。**两者不得混同，也不得借本次变更
恢复旧体系。**
...
当本次填充是**返修**（`revisionRound > 0`）时，上下文追加（D-31）：

```text
+ 上一轮 Agent 的公开输出与工具调用记录
+ 上一轮提交的 Slot Content
+ 上一轮通过引文校验的 Review Finding
```

这三项均为**任务运行期数据**，与模板/技能无关，因此进 Context 而非冻结快照
（AC-002 不受影响）。追加项同样参与 `contextHash` 计算（FR-CTX-006）——
不同返修轮次的 Context 内容不同，Hash 因此不同，这是正确的：任务状态本就不同。
...
**返修轮次不构成本条的例外（D-31）。** 同一槽位的返修，其 Agent 上下文是
**连续的**——上一轮的公开输出与结论会出现在下一轮的上下文里。但这种连续性
必须**完全由重建产生**，而不是由跨 Assignment 存活的会话对象产生：

- 上一轮的公开输出：从该 Execution 记录中读出；
- 上一轮读过哪些依赖槽位：记录槽位 ID，**内容在重建时从数据库现取**，
  不做副本（副本会与库里的权威内容漂移，也会白白撑大上下文）；
- 上一轮的审核结论：从审核结果记录中读出。

判定标准仍是本条最后一句：**清空进程内存后，只凭数据库与冻结快照，
必须能重建出逐字相同的上下文。** 做不到就是违反本条。
...
**以上校验全部是确定性的，不含任何语义判断**——本条的性质不变。

语义审核作为**独立的后续步骤**由 FR-REVIEW-001..004 规定，不并入本条。
两者的区别是刚性的，实现时不许混淆：

- **本条（确定性校验）失败** ⇒ 提交被拒，内容不落库；
- **语义审核检出问题** ⇒ 内容已落库，走返修流程。

且语义审核**不构成质量保证**：它按判据找错，找不到不等于内容合格（FR-REVIEW-004）。
...
5. 设置 Slot 状态：
   - Slot Type **未绑定** Review Skill ⇒ `completed`；
   - Slot Type **已绑定** Review Skill ⇒ `reviewing`（FR-REVIEW-001）；
...
- Slot 处于 `reviewing`，但下游 Slot 已开始执行。

---

### FR-REVIEW-001 语义审核的触发与范围

Slot Type 可选绑定一个 Review Skill。**未绑定的 Slot Type 不进行任何语义审核。**

已绑定时，Slot 内容通过 FR-SLOT-004 校验并落库后进入 `reviewing`，
由 Review Agent 按 Review Skill 声明的**每一条判据各执行一次独立 Assignment**
进行审核。审核**不是**槽位完成之后的独立流程，而是槽位完成的**组成部分**：
Slot 必须审核结算完毕才可能进入 `completed`，因此下游 Slot 在此期间不得开始
（FR-SCH-001 的依赖判定不变，`reviewing` 天然不满足「依赖已 completed」）。

判据清单由 Review Skill 定义，**本规格不规定判据内容，也不限制其数量**。
判据的增删属于 Skill 迭代，不构成对本规格的变更。

Review Assignment 与 Fill Slot Assignment 共用同一条 Task Active Execution
单车道，同受 Execution Token 约束（FR-AGT-002）与迟到结果拒绝（FR-LIFE-002）。

### FR-REVIEW-002 审核结论的采纳

Review Agent 返回判定与 Finding。**系统不直接执行 Agent 的判定。**

每条 Finding 必须携带一段取自待审内容的引文。系统以**确定性比对**
（允许标点归一化）校验该引文确实出现在待审内容中：

- 校验不通过的 Finding 一律丢弃，并记录丢弃数量；
- 一次判定的 Finding **全部**丢弃时，该判据按「未检出问题」处理；
- Review Assignment 重试耗尽或返回无法解析时，该判据同样按「未检出问题」
  处理，并记录其原因是**审核失败**而非未检出。

最终的 Slot 状态由系统依据结算规则写定，Agent 无权翻转 Slot 状态（AC-014）。

### FR-REVIEW-003 返修与终止性

检出问题且返修轮次未达上限时，Slot 回到 `pending` 重新生产。
**返修必须保留上一轮内容**，供下一轮上下文使用（FR-CTX-002）——
这与 FR-LIFE-001 停止所致的 `running → pending`（不保留内容）语义相反。

返修轮次上限来自 Slot Type 定义。**达到上限后 Slot 以当前内容进入 `completed`**，
并记录其为「返修次数用尽，按现状完成」。

系统必须保证：

- 任务不得因语义审核进入永久 Running；
- 任务不得因语义审核进入 `failed`；
- 返修不消耗 FR-AGT-004 的故障重试预算，两者是不同的计数。

### FR-REVIEW-004 审核结论的表述

语义审核**不构成内容质量保证**。它只回答「按现有判据是否找到了问题」，
找不到问题**不等于**内容合格。

因此系统在 Trace、API 与前端中：

- 必须使用「未检出问题」一类表述；
- **不得**使用「审核通过」「质量合格」「已校验」一类使人推断内容质量
  已获保障的表述。

本条属于正确性要求而非文案偏好：每条判据的实际检出能力各不相同，
其中部分判据可能长期检不出任何问题。以「通过」表述之即为不实陈述。

每条判据的实测检出能力应单独记录并可被持续观察，以便 Skill 与模型的改进
能够被验证（见设计文档 D-28）。
...
本条对**返修上下文与 Review Finding 同样有效**，且此处最易疏漏：
返修上下文携带上一轮 Agent 的输出（FR-CTX-002），Review Finding 携带模型自由文本，
两者都会落入数据库。序列化前必须走与其他 Agent 输出**相同**的过滤路径，
不得为语义审核另开一条。
...
2. 无任务永久停留在 Running（含因语义审核或返修而停留，见 FR-REVIEW-003）；
3. 无停止后的迟到结果被提交；
4. 无部分结构提交；
...
9. 核心生产链**不依赖**语义审核即可完成——未绑定 Review Skill 的 Slot Type
   完全不走审核路径（FR-REVIEW-001），审核故障也不阻断内容生产（FR-REVIEW-002）；
   不依赖 Finding、Seal 或 Capability 模块；
...
P0 的自动返修（FR-REVIEW-003）已经实现了其中的**机制**部分——
保留旧内容、基于上一版本进行局部修订。本阶段剩余的是**人工入口**：

- 手动重生成单个 Slot；
- 用户填写返修指令；
- Slot Revision（多版本留存与查看；P0 只保留上一稿供返修使用，不建版本历史）；
- 重新组装 Artifact。
...
**Slot Review 与局部返修已提前进入 P0**（见 §7.1 与 FR-REVIEW-001..004），
不再属于本阶段。
...
- 审核返回受影响 Slot（跨槽位影响面判定）。

> P0 的做法刻意回避了「受影响 Slot」这个问题：审核是槽位完成的组成部分，
> 下游只会读到已审结算过的内容，因此不存在「上游改了下游要不要重做」。
> 只有当审核扩展到跨槽位范围时，本项才重新成为必要。

**不默认恢复现有 Finding、Repair Grant 和权威审核体系**——
P0 的 Review Finding 是一个刻意轻量的独立概念，与之无关（见 §7.1 术语澄清）。
