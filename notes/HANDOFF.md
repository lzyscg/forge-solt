...
| R0–R6 审核返修 | ✅ | 审核链、编辑清单返修、Provider 降级链（见 `AUTOMATED-REVIEW-REVISION-DESIGN-V0.2.md`） |
| **S0–S6 内容质量** | 🚧 **进行中** | 见下方「当前阶段」 |

### 当前阶段：S0–S6，Skill 与模板的内容质量迭代

平台本身（M0–M7 + R0–R6）已经能跑。**现在做的是产出质量，不是功能。**

真实业务目标此前不在任何文档里：**产出要能投稿并被知乎/番茄/起点签约**，
因此必须没有 AI 味。→ `notes/SKILL-ITERATION-PLAN-S0-S6.md` 是这一阶段的**防跑偏文档**，
换会话先读它；决策与论证在 `notes/SKILL-ITERATION-BENCH-DESIGN-V0.1.md`（D-71…D-88）。

三条对接手最要紧的：

1. **`知乎短故事示例/` 与 `agent-skills原始/` 已 gitignore。** 它们是知乎盐选**付费作品**，
   多篇自带「下载后 24 小时内删除」声明，**而本仓库是公开的**。范文正文一个字都不能进仓库。
   只有 `fixtures/reference-corpus.json`（153 篇的纯数字度量 + 文件名，28KB）可以入库。
2. **指纹是闸门，不是分数。** 「本轮指纹提升了 12%」是一句无意义的话（D-75）。
3. **S0–S2 是建尺子，期间不许改 `skills/` 下任何一个字**——否则基线和被测对象同时在动。
...
全文在 `notes/OPEN-QUESTIONS.md`，共 **28 条**。Q-21 与 Q-23 已在 M7 落实，
**Q-24 已由业务方定案**（各自条目下都补了结论）。

S 阶段新增的一条（**这条是缺陷**）：

- **Q-28** 生产模板与 `skills/` 之间的耦合只有一半机器可读。加载期只校验
  `frontmatter.slotTypes` ↔ 绑定的槽位类型（`template-loader.ts:446`）；
  Skill **正文**里「用 `read_slot` 读章节骨架」是散文，检查不了；
  `skills/` 下的孤儿 Skill 也看不见（校验从模板出发）。
  结果：三个在用的 Skill 指挥模型去读一个生产模板里**从初始提交起就不存在**的
  `chapter_outline` 槽位，靠人工比对才发现。处置见 PLAN §3.4b ④⑤（D-86…D-88）。
